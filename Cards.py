...
# ─── Utilitaires ──────────────────────────────────────────────────
def tirer_carte():
    """Tire une carte selon les probabilités de rareté."""
...
def build_frame_animation(frame_num: int) -> str:
    """Génère une frame d'animation pour le spin."""
...
class Cards(commands.Cog):
    ...
    async def cardspin(self, ctx):
        """Dépense 100 pièces pour tirer une carte aléatoire."""
        if SALON_BOUTIQUE not in ctx.channel.name.lower():
            boutique = discord.utils.get(ctx.guild.text_channels, name__contains=SALON_BOUTIQUE)
    ...
    async def collection(self, ctx, member: discord.Member = None):
        """Affiche la collection de cartes d'un membre."""
    ...
    async def cartesinfo(self, ctx):
        """Affiche toutes les raretés et leurs probabilités."""
        lignes = []
