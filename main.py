...
async def send_help(channel):
    ...
    if "jeux" in channel_name:
        ...
        embed.description = (
            "**Profil & Stats**\n"
            "`!profil` / `/profil` — niveau, pièces, rôles\n"
            "`!inventaire` / `/inventaire` — tes rôles achetés\n"
            "`!classement` / `/classement` — top 10\n"
            "`/solde` — solde rapide\n\n"
            "**Social**\n"
            "`!parrainer @pseudo` / `/parrainer` — parrainer un ami\n"
            "`!collection` / `/collection` — ta collection de cartes\n\n"
            "**Préférences**\n"
            "`/notif on/off` — notifications level up\n\n"
