...
async def handle_member_resolution(channel, action_data, author_id, exact, similar, is_id=False, is_banned=False):
    ...
    if is_id and len(exact) == 1:
        ...
        if is_banned:
            user = exact[0]
            embed = discord.Embed(title=f"🔨 {user.display_name} est banni", description="Cet utilisateur est actuellement banni du serveur.", color=0xe74c3c)
    ...
    if len(all_candidates) == 1:
        ...
        m = all_candidates[0]
        embed = discord.Embed(title="🔍 Membre similaire trouvé", description=f"Voulais-tu dire **{m.display_name}** ?", color=0xf39c12)
...
async def on_message(message):
    ...
    if "jeux" in channel_name:
        from economy import cmd_profil, cmd_inventaire, cmd_classement, cmd_parrainer
        ...
        if content_lower.startswith("!parrainer"):
            ...
            return
        if content_lower in ["!boutique", "!spin", "!cardspin"] or content_lower.startswith("!acheter") or content_lower.startswith("!équiper"):
    ...
    if "boutique" in channel_name:
        from economy import cmd_boutique, cmd_acheter, cmd_equiper, cmd_spin
